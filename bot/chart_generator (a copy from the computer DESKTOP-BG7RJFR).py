...
def get_available_presets(server_url="http://localhost:3002/api/presets?approved=true"):
    """Get list of available presets from main server (server.js)"""
    import logging
    logger = logging.getLogger(__name__)
    
    try:
        logger.info(f"Fetching approved presets from main server: {server_url}")
        response = requests.get(server_url, timeout=5)
        
        if response.status_code == 404:
            logger.error(f"❌ API endpoint not found (404): {server_url}")
            logger.error("Please check that server.js is running and the /api/presets endpoint is available")
            return []
        
        response.raise_for_status()
        result = response.json()
        
        # server.js returns { presets: [...] } format
        if "presets" in result and isinstance(result["presets"], list):
            presets = [preset.get("name") for preset in result["presets"] if preset.get("name")]
            logger.info(f"✅ Retrieved {len(presets)} approved presets from server: {presets}")
            return presets
        else:
            logger.error(f"❌ API response error: unexpected format")
            logger.error(f"Full API response: {result}")
            return []
    except requests.exceptions.ConnectionError as e:
        logger.error(f"❌ Cannot connect to main server at {server_url}")
        logger.error("Please check that server.js is running (default port 3002)")
        return []
    except Exception as e:
...
def get_next_random_preset(config, server_url="http://localhost:3002/api/presets?approved=true", script_dir=None):
    """
