...
def schedule_next_post(guild_id, config=None):
    ...
    try:
        ...
        # Migration: if duration is 0 in DB but exists in JSON config, migrate it
        if config:
            ...
            if duration == 0 and json_duration > 0:
                logger.info(f"Migrating duration from JSON to DB for guild {guild_id}: {json_duration} hours")
                db_manager.set_duration(guild_id, json_duration)
                duration = json_duration
            
            # Migrate last_poster_time if needed (and if DB has 0)
            if json_last_poster_time > 0 and last_poster_time == 0:
                logger.info(f"Migrating last_poster_time from JSON to DB for guild {guild_id}: {json_last_poster_time}")
                db_manager.set_last_poster_time(guild_id, json_last_poster_time)
...
async def auto_post_chart(guild_id):
    ...
    try:
        ...
        group_data = db_manager.get_group_data(guild_id)
        duration = group_data.get('duration', 0)
        logger.info(f"Config loaded for guild {guild_id}, duration: {duration} (from DB)")
        ...
        for filename in os.listdir(GUILDS_DIR):
            ...
            try:
                ...
                if config:
                    ...
                    # Migrate last_poster_time if needed
                    ...
                    if not group_data.get('free', False) and json_free:
                        logger.info(f"Migrating free from JSON to DB for guild {guild_id}: {json_free}")
                    ...
                    if group_data.get('minimal_duration', 0.02) == 0.02 and json_minimal_duration != 0.02:
                        logger.info(f"Migrating minimal_duration from JSON to DB for guild {guild_id}: {json_minimal_duration}")
                        db_manager.set_minimal_duration(guild_id, json_minimal_duration)
                
                if duration > 0:
                    logger.info(f"Creating scheduled task for guild {guild_id} with duration {duration} hours")
                    schedule_next_post(guild_id)
                    task_counter += 1
                else:
...
async def generate_chart_interaction(interaction: discord.Interaction, is_auto=False):
    ...
    try:
        ...
        current_time = time.time()
        # Get last_poster_time from database
        group_data = db_manager.get_group_data(guild_id)
        last_poster_time = group_data.get('last_poster_time', 0)
        
        # Migration: if last_poster_time is 0 in DB but exists in JSON config, migrate it
        if config:
            json_last_poster_time = config.get('last_poster_time', 0)
            if json_last_poster_time > 0 and last_poster_time == 0:
                logger.info(f"Migrating last_poster_time from JSON to DB for guild {guild_id}: {json_last_poster_time}")
        ...
        minimal_duration = group_data_min.get('minimal_duration', 0.02)
        
        # Migration: if minimal_duration is default in DB but exists in JSON config, migrate it
        if config:
            json_minimal_duration = config.get('minimal_duration', 0.02)
            if minimal_duration == 0.02 and json_minimal_duration != 0.02:
                logger.info(f"Migrating minimal_duration from JSON to DB for guild {guild_id}: {json_minimal_duration}")
                db_manager.set_minimal_duration(guild_id, json_minimal_duration)
        ...
        if render_chart(config, ohlcv_data, token_info, output_file):
            ...
            # Update last_poster_time in database
            db_manager.set_last_poster_time(guild_id, current_time)
            
            # Save config (without duration and last_poster_time as they're in DB)
            settings_manager.save_guild_config(guild_id, config)
            
            # Reschedule the task only for manual posts (not auto posts)
            if not is_auto:
                group_data_duration = db_manager.get_group_data(guild_id)
                if group_data_duration.get('duration', 0) > 0:
                    schedule_next_post(guild_id)
...
class FreeStatusButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Get current free status from database
        group_data = db_manager.get_group_data(self.guild_id)
        current_free = group_data.get('free', False)
        new_free = not current_free
        
        # Save free to database instead of JSON
        db_manager.set_free(self.guild_id, new_free)
        
        # Remove free, duration, last_poster_time from config before saving to JSON
        if 'free' in config:
            del config['free']
...
class DurationModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        db_manager.set_duration(self.guild_id, duration)

        # Remove duration from config before saving to JSON
        if 'duration' in config:
            del config['duration']
        if 'last_poster_time' in config:
            del config['last_poster_time']
        ...
        if settings_manager.save_guild_config(self.guild_id, config):
            ...
            if duration > 0:
                ...
                schedule_next_post(self.guild_id)
...
class MinimalDurationModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        db_manager.set_minimal_duration(self.guild_id, min_duration)
        
        # Remove minimal_duration and other DB fields from config before saving to JSON
