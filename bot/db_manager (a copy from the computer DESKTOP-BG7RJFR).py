...
class DatabaseManager:
            ...
            with self._get_cursor() as cursor:
                cursor.execute("""
                    CREATE TABLE IF NOT EXISTS tg_groups (
                        ...
                        minimal_duration FLOAT DEFAULT 2,
    ...
    def _migrate_columns(self):
        ...
        try:
            with self._get_cursor() as cursor:
                ...
                # Проверяем и добавляем minimal_duration
                try:
                    cursor.execute("SHOW COLUMNS FROM tg_groups LIKE 'minimal_duration'")
                    if not cursor.fetchone():
                        cursor.execute("ALTER TABLE tg_groups ADD COLUMN minimal_duration FLOAT DEFAULT 2")
                        logger.info("✅ Added minimal_duration column to tg_groups table")
                    else:
                        logger.debug("Column minimal_duration already exists")
                except Exception as e:
                    logger.warning(f"Error checking/adding minimal_duration column: {e}")
        except Exception as e:
            logger.error(f"Error during column migration: {e}")
    
    def get_group_data(self, group_id):
        """Получить данные группы (last_poster_time, duration, minimal_duration, random_chart, free)"""
        group_id_str = str(group_id)
        try:
            with self._get_cursor() as cursor:
                cursor.execute(
                    "SELECT last_poster_time, duration, minimal_duration, random_chart, free FROM tg_groups WHERE group_id = %s",
                    (group_id_str,)
                )
                ...
                if result:
                    return {
                        ...
                        'minimal_duration': result.get('minimal_duration', 2),
                    ...
                    self.set_group_data(group_id_str, last_poster_time=0, duration=0, minimal_duration=2, random_chart=False, free=False)
                    return {'last_poster_time': 0, 'duration': 0, 'minimal_duration': 2, 'random_chart': False, 'free': False}
        except Exception as e:
    ...
    def set_minimal_duration(self, group_id, minimal_duration):
        """Установить minimal_duration для группы"""
        group_id_str = str(group_id)
        try:
            with self._get_cursor() as cursor:
                cursor.execute("""
                    INSERT INTO tg_groups (group_id, minimal_duration)
                    VALUES (%s, %s)
                    ON DUPLICATE KEY UPDATE
                        minimal_duration = VALUES(minimal_duration)
                """, (group_id_str, float(minimal_duration)))
                logger.debug(f"Set minimal_duration={minimal_duration} for group {group_id_str}")
        except Exception as e:
            logger.error(f"Error setting minimal_duration for {group_id}: {e}")
    
    def set_group_data(self, group_id, last_poster_time=None, duration=None, minimal_duration=None, random_chart=None, free=None):
        ...
        try:
            with self._get_cursor() as cursor:
                ...
                if minimal_duration is not None:
                    updates.append("minimal_duration = %s")
                    insert_fields.append("minimal_duration")
                    insert_values.append(float(minimal_duration))
