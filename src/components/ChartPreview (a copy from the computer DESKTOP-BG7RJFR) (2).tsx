...
const ChartPreview: React.FC<ChartPreviewProps> = ({
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent | TouchEvent) => {
      ...
      if (!item || hit === 'none') {
        onSelectedOverlayChange && onSelectedOverlayChange(null);
        drawOverlays();
        return;
      }
      onSelectedOverlayChange && onSelectedOverlayChange(item.id);
    ...
    const onMove = (e: MouseEvent | TouchEvent) => {
      if (!overlays) return;
      if (!start || !dragStartRef.current || dragModeRef.current === 'none') return;
      const { x, y } = toCanvasCoords(e);
      ...
      if (dragModeRef.current === 'move') {
        ...
        next[idx] = {
          ...current,
          mirrored: current.mirrored ?? startPos.item!.mirrored ?? false,
          x: (startPos.item!.x + dx),
          y: (startPos.item!.y + dy)
        };
        onOverlaysChange && onOverlaysChange(next);
      } else if (dragModeRef.current === 'scale') {
        const dx = x - current.x; const dy = y - current.y;
        const dist = Math.hypot(dx, dy);
        const sdx = startPos.x - current.x; const sdy = startPos.y - current.y;
        ...
        next[idx] = {
          ...current,
          mirrored: current.mirrored ?? startPos.item!.mirrored ?? false,
          scale: Math.max(0.05, (startPos.item!.scale || 1) * factor)
        };
        onOverlaysChange && onOverlaysChange(next);
        ...
        const sangle = Math.atan2(startPos.y - current.y, startPos.x - current.x);
        const delta = angle - sangle;
        const next = overlays.slice();
        next[idx] = {
          ...current,
          mirrored: current.mirrored ?? startPos.item!.mirrored ?? false,
          rotation: (startPos.item!.rotation || 0) + delta
        };
        onOverlaysChange && onOverlaysChange(next);
      }
      drawOverlays();
