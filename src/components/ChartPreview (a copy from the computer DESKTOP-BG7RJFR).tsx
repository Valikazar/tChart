...
const ChartPreview: React.FC<ChartPreviewProps> = ({
  ...
  const hitTest = (px: number, py: number) => {
    ...
    for (let i = overlays.length - 1; i >= 0; i--) {
      ...
      // Calculate distance to all handles and find the closest one
      const allHandles = [...corners, mirror];
      const handleDistances = allHandles.map(h => ({
        ...h,
        distance: Math.hypot(px - h.x, py - h.y)
      }));
      const inCircle = (cx: number, cy: number, r = HIT_RADIUS) => (px - cx) * (px - cx) + (py - cy) * (py - cy) <= r * r;
      
      // Find closest handle within hit radius
      const validHandles = handleDistances.filter(h => inCircle(h.x, h.y));
      if (validHandles.length > 0) {
        // Sort by distance and return the closest one
        validHandles.sort((a, b) => a.distance - b.distance);
        return { item, hit: validHandles[0].key };
      }
  ...
  useEffect(() => {
    ...
    const onDown = (e: MouseEvent | TouchEvent) => {
      ...
      const isCurrentlySelected = selectedIdRef.current === item.id;
      
      // If clicking on an unselected image, activate it first instead of executing handle actions
      if (!isCurrentlySelected) {
        selectedIdRef.current = item.id;
        dragModeRef.current = hit === 'move' ? 'move' : hit === 'scale' ? 'scale' : hit === 'rotate' ? 'rotate' : 'none';
        // For direct-action handles, we initialize drag only if it's a drag operation
        if (dragModeRef.current !== 'none') {
          dragStartRef.current = { x, y, item: { ...item } };
        }
        // Just activate the image, don't execute handle actions on first click
        drawOverlays();
        e.preventDefault();
        return;
      }
      
      // Image is already selected, so we can execute handle actions
      selectedIdRef.current = item.id;
      dragModeRef.current = hit === 'move' ? 'move' : hit === 'scale' ? 'scale' : hit === 'rotate' ? 'rotate' : 'none';
