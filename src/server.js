...
app.use(express.static('build')); // Assuming the built React app is in the build directory
app.use(morgan('dev')); // Request logging
app.use('/pic', express.static(path.join(__dirname, 'pic'))); // Serve static files for images
...
(async () => {
  ...
  try {
    ...
    `);
    console.log('Added public column to presets table');
  } catch (e) {
    // Column already exists, ignore error
    if (!e.message.includes('Duplicate column name')) {
      console.error('Error adding public column:', e);
    }
  }
  ...
  try {
    ...
    `);
    console.log('✅ Table subreddit_settings created successfully');
    ...
    console.error('❌ Error creating subreddit_settings table:', error);
    throw error;
  }
})();
...
app.get('/api/images/:category', async (req, res) => {
  ...
  const validCategories = ['body', 'center', 'topbot', 'bg', 'presets'];
  ...
  try {
    ...
    for (const image of images) {
      ...
      if (category === 'bg') {
        fileExtension = '.jpg';
      } else if (['center', 'topbot', 'body'].includes(category)) {
        fileExtension = '.png';
      } else {
        fileExtension = '.png'; // По умолчанию PNG для других категорий
      }
...
app.get('/api/genres', async (req, res) => {
  try {
    ...
    const categories = ['body', 'center', 'topbot', 'bg'];
...
app.post('/api/save-subreddit-settings', async (req, res) => {
  try {
    const { subreddit, settings } = req.body;
    ...
    const [result] = await mariadbPool.query(`
      INSERT INTO subreddit_settings (subreddit, user_address, settings) 
      VALUES (?, ?, ?)
    `, [subreddit, 'global', JSON.stringify(settings)]);
...
app.get('/', (req, res) => {
  console.log('Handling root route with query params:', req.query);
  res.sendFile(path.join(__dirname, 'build', 'index.html'));
});

// Обязательно ограничиваем маршрут, чтобы он не перехватывал /api/*
app.get(/^(?!\/api\/).+$/, (req, res) => {
  console.log('Handling frontend route for:', req.url);
  res.sendFile(path.join(__dirname, 'build', 'index.html'));
});
...
app.listen(PORT, () => {
  ...
  console.log(`API available at: http://localhost:${PORT}/api/images/:category`);
  console.log(`Groups directory: ${GROUPS_DIR}`);
  // Check groups directory existence
  if (!fs.existsSync(GROUPS_DIR)) {
    console.log('WARNING: Groups directory does not exist!');
    try {
      fs.mkdirSync(GROUPS_DIR, { recursive: true });
      console.log('Created groups directory');
    } catch (err) {
      console.error('Failed to create groups directory:', err);
    }
  } else {
    // Show list of configuration files
    const files = fs.readdirSync(GROUPS_DIR);
    console.log('Available group configurations:', files);
  }
});
