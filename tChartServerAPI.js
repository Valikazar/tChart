...
const imagesDir = path.join(__dirname, 'img', 'temp');
...
app.get('/', (req, res) => {
    res.json({
        ...
        endpoints: {
            ...
            'GET /configs': 'Список доступных конфигураций',
...
// Эндпоинт для получения списка доступных конфигураций
app.get('/configs', (req, res) => {
    try {
        // Проверяем оба возможных пути (если файл в корне или в dist/)
        let presetsDir = path.join(__dirname, 'pic', 'presets');
        if (!fs.existsSync(presetsDir)) {
            presetsDir = path.join(__dirname, '..', 'pic', 'presets');
        }

        if (!fs.existsSync(presetsDir)) {
            console.error(`Папка с конфигурациями не найдена. Проверялись пути: ${path.join(__dirname, 'pic', 'presets')} и ${path.join(__dirname, '..', 'pic', 'presets')}`);
            return res.status(404).json({ error: 'Папка с конфигурациями не найдена' });
        }
        ...
        res.status(500).json({ error: 'Ошибка при получении списка конфигураций' });
    }
});
...
app.post('/render', async (req, res) => {
    try {
        ...
        const imageFormat = format && ['png', 'jpg', 'jpeg', 'webp'].includes(format.toLowerCase()) ? format.toLowerCase() : 'png';
        const fileExtension = imageFormat === 'webp' ? 'webp' : (imageFormat === 'jpg' || imageFormat === 'jpeg' ? 'jpg' : 'png');
